...
{
    public partial class BaoTriSuKien : Form
    {
        //tạo 2 biến cục bộ
        ...
        private void BaoTriSuKien_Load(object sender, EventArgs e)
        {
            ...
            reader.Close();
            dgvBaoTriSuKien.DataSource = listSK;
        }
        ...
        {
            this.Hide();
            SuKienChuaDuyet skcd = new SuKienChuaDuyet();
            skcd.ShowDialog();
        }

        private void dgvBaoTriSuKien_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex == -1) return;
            SuKien sk = new SuKien();
            DataGridViewRow row = dgvBaoTriSuKien.Rows[e.RowIndex];
            ...
            sk.tinhTrangThanhToan = row.Cells[5].Value.ToString();
            sk.ghiChu = row.Cells[6].Value.ToString();
            sk.trangThai = row.Cells[7].Value.ToString();
